...
log_file = "api/logs/test-nifty-t.log"
...
def get_all_thursday_expiries(year, month, today=None, num_expiries=5):
    """
    Get the next 5 Thursday expiries starting from the current month.
    If current month doesn't have enough expiries, fetch from next month(s).
    
    Args:
        year (int): Initial year to start looking for expiries
        month (int): Initial month to start looking for expiries
        today (date, optional): Current date for reference. Defaults to None.
        num_expiries (int): Number of expiry dates to fetch. Defaults to 5.
    ...
    logging.info(f"Calculating {num_expiries} expiries starting from {year}-{month}, Today: {today}")
    ...
    while len(all_valid_expiries) < num_expiries:
        # Get all Thursdays of the current month
        ...
        for day in range(1, num_days + 1):
            ...
            if current_date.weekday() == 3:  # Thursday is 3
                thursdays.append(current_date.date())
        
        logging.info(f"All Thursdays in {current_year}-{current_month}: {thursdays}")
        ...
        # Adjust expiry dates for holidays
        for thursday in thursdays:
            expiry_date = thursday
            
            # If Thursday is a holiday, move to previous trading day
            while expiry_date.strftime('%Y-%m-%d') in holidays_list or expiry_date.weekday() > 4:
                expiry_date -= timedelta(days=1)
            
            # Only include expiries that are today or in the future
        ...
        # Move to next month if we still need more expiries
        if current_month == 12:
            current_month = 1
            current_year += 1
        else:
    ...
    logging.info(f"Found {len(all_valid_expiries)} valid expiry dates: {all_valid_expiries}")
...
class OptionChainFetcher:
    def fetch_data(self):
        ...
        # Get all valid Thursday expiries
        ...
        for expiry_date in expiry_dates:
            ...
            try:
                ...
                if data.get('status') == 'success':
                    ...
                    logging.info(f"Successfully fetched data for expiry date: {expiry_date}")
                
                # Add a small delay between requests to avoid rate limiting
                t.sleep(0.5)
                
            except requests.RequestException as e:
                logging.error(f"Request failed for expiry {expiry_date}: {e}")
        
        return all_data

    def start_fetching(self):
        logging.info("Starting data fetching for all expiries...")
        all_expiry_data = self.fetch_data()
        
        for expiry_data in all_expiry_data:
            expiry_date = expiry_data['expiry_date']
            data_items = expiry_data['data']
            ...
            for item in data_items:
                ...
                # Generate table name using the expiry date
                table_name = sanitize_table_name(str(expiry), underlying_key)
