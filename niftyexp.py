...
log_file = "api/logs/test-nifty.log"
...
def get_all_thursday_expiries(year, month, today=None):
    """
    Get all Thursday expiries for a given month, including today's expiry if applicable.
    Returns a list of expiry dates.
    """
    if today is None:
        today = datetime.now(pytz.timezone('Asia/Kolkata')).date()
    
    logging.info(f"Calculating expiries for {year}-{month}, Today: {today}")
    ...
    logging.info(f"All Thursdays in month: {thursdays}")
    ...
    # If today is an expiry day, include it
    if today in [exp for exp in valid_expiries]:
        logging.info(f"Today ({today}) is an expiry day")
    
    # If all expiries for current month are passed, get next month's expiries
    if not valid_expiries:
        next_month = datetime(year, month, 1) + timedelta(days=32)
        logging.info(f"No valid expiries found in current month, checking next month: {next_month.year}-{next_month.month}")
        return get_all_thursday_expiries(next_month.year, next_month.month, today)
    
    logging.info(f"Valid expiry dates: {valid_expiries}")
