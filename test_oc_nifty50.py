    ...
    for week in c:
        ...
        if thursday != 0:  # 0 means the day belongs to another month
            ...
            if thursday_date.strftime('%Y-%m-%d') in holidays_list:
                ...
                logging.info(f"Holiday found on {thursday_date}, adjusted to {temp_date}")
            else:
...
def get_next_thursday(year, month, today=None):
    """Modified to handle today's expiry correctly"""
    if today is None:
        today = datetime.today()

    thursday_dates = get_all_thursday_expiries(year, month)
    
    # If today is an expiry date, return today's date
    today_date = today.date()
    if today_date in thursday_dates:
        logging.info(f"Today {today_date} is an expiry day")
        return today_date

    # Find the next available Thursday
    for thursday_date in thursday_dates:
        if thursday_date > today_date:
            logging.info(f"Next Thursday expiry date is {thursday_date}")
            return thursday_date

        # If no Thursday found in current month, get first Thursday of next month
        if month == 12:
            next_month_dates = get_all_thursday_expiries(year + 1, 1)
        else:
            next_month_dates = get_all_thursday_expiries(year, month + 1)
    
        return next_month_dates[0] if next_month_dates else None
...
class OptionChainFetcher:
        ...
        for expiry in expiry_dates:
            ...
            try:
                ...
                if data.get('status') == 'success':
                    # Add expiry date to the data for later use
    ...
    def start_fetching(self):
        ...
        for data_batch in all_data:
            if data_batch.get('status') == 'success' and 'data' in data_batch:
                expiry_date = data_batch['expiry_date']  # Get the expiry date we stored earlier
                
                for item in data_batch['data']:
                    underlying_key = item.get('underlying_key')
                    table_name = sanitize_table_name(str(expiry_date), underlying_key)
                    
                    print(f"{Fore.GREEN}Inserting data for {expiry_date} into {table_name}{Style.RESET_ALL}")
                    ...
                    try:
                        ...
                        error_msg = f"Error inserting data for {expiry_date}: {e}"
                        logging.error(error_msg)
                        print(f"{Fore.RED}{error_msg}{Style.RESET_ALL}")
            else:
